        ...
        antennas.foreach{ (antennaType, positions) =>
            positions.foreach{ position =>
                positions.diff(Set(position)).foreach{ otherPosition =>
                    ...
                    var nextAntenodeForward = otherPosition + distance
                    while(grid.validPosition(nextAntenodeForward))
                        antenodes.setAntenode(nextAntenodeForward)
                        nextAntenodeForward = nextAntenodeForward + distance

                    var nextAntenodeBackward = position - distance
                    while (grid.validPosition(nextAntenodeBackward))
                        antenodes.setAntenode(nextAntenodeBackward)
                        nextAntenodeBackward = nextAntenodeBackward - distance

                    // Mark the antenas as antenodes
                    antenodes.setAntenode(position)
                    antenodes.setAntenode(otherPosition)
                }
            }
        }
